...
public class IndexController extends AbstractController {
    ...
    public String about(HttpServletRequest request) {
        List<MetaVo> about = metaService.getMetas(Types.ABOUT.getType());
...
//        对于空格的特殊处理
     ...
     * @return
     */
    @GetMapping(value = "comm/index")
    public String lllm(HttpServletRequest request) {
        return "comm/index";
    }
    /**
     * IP被禁后的页面
    ...
    @ResponseBody
    public ResponseVo testRedis(HttpServletRequest request) {
        logger.error("testredis start");
        String key = request.getParameter("key");
        String value = request.getParameter("value");
        logger.error("testredis key = " + key + ", value = " + value );
        redisService.set(key,value);
        logger.error("testredis set end");
        String ret = redisService.get(key);
        logger.error("testredis end = " + ret );

        ResponseVo vo = new ResponseVo();
        vo.setData("key = " + key + ", value = " + ret);
        vo.setCode("200");
        vo.setMsg("success");
        return vo;
    }
